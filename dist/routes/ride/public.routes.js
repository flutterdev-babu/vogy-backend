...
router.get("/fare-estimate", async (req, res) => {
    try {
        ...
        let vehicleTypes = await prisma_1.prisma.vehicleType.findMany({
            where: { isActive: true },
            orderBy: { pricePerKm: "asc" },
        });
        // If location provided, filter vehicle types by nearby online riders
        let nearbyRidersCount = {};
        if (hasLocationFilter) {
            const userLat = parseFloat(pickupLat);
            const userLng = parseFloat(pickupLng);
            // Get all online partners with their vehicle types
            const onlinePartners = await prisma_1.prisma.partner.findMany({
                where: {
                    isOnline: true,
                    currentLat: { not: null },
                    currentLng: { not: null },
            ...
            const nearbyVehicleTypeIds = new Set();
